...
2. **One bounded execution primitive.** Every bounded project command SenpAI executes is a capsule: a deterministic argv template, optional working directory, optional agent-supplied parameters, bounded output, and a timeout. SenpAI executes it without a shell. Capsules may require no local values; when private placeholders exist, SenpAI resolves and scrubs them inside its own process. Rich external platforms such as trackers and code hosts remain driven by skills. Interactive shells, foreground servers, follow-mode logs, and other unbounded processes are intentionally unsupported.
