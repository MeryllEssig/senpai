...
- File name: `.senpai.jsonc` (decided).
...
- A published JSON Schema for editor completion and structural CLI validation:
  [`schema/senpai.schema.json`](../schema/senpai.schema.json). It is
...
- **CLI mechanism.** `senpai resolve` returns the canonical absolute manifest
...
- **`senpai-project-management`** handles ticket capabilities: `read`,
  `create`, `update`, `comment`, `transition`, `link`, and `log_time`.
- **`senpai-code-hosting`** handles code-change capabilities: `read`,
...
- **Execution-network boundary.** A tracker or hosting workflow permission authorizes the project operation; it does not authorize outbound network access in the agent's execution environment. Adapters and hosting CLIs know when they contact a declared remote target and request any environment-level network approval before the call. They never treat empty or incomplete command output as evidence of a remote result.
...
- **Optional structured `access` hint (decided 2026-07-20).** A lightweight `"access": "read-only"` may be declared on a capsule as information for the LLM. It is advisory and never enforced, including during `senpai run`.

### 1.9 Local overlay (personal, gitignored)

A `.senpai.local.jsonc` sitting **beside** the resolved `.senpai.jsonc` (same directory) provides a personal, gitignored overlay for private paths, personal preferences, personal `auth` overrides, and personal capsule overrides. Compatibility rules (decided):
...
- **Never a home for secrets.** The overlay deep-merges into the manifest, so the agent reads it like the manifest itself. This is why capsule values live in a separate file the agent never reads (`.senpai/capsules.local.json`, 1.10) rather than in the overlay.
...
- **CLI additions and onboarding** (see 2.1):
  - When at least one capsule has a non-supplied placeholder, `init` creates
    `.senpai/capsules.local.json` with one stub per such variable, creates
    `.senpai/.gitignore` containing the values-file exclusion, and restricts
...
- **File topology (flat siblings):**
  ...
  - `.senpai/capsules.local.json` - optional local capsule-values file,
...
- **init**: scaffold the local capsule-values file (`.senpai/capsules.local.json`) from the manifest's capsules - one stub per non-supplied `{variable}` (see 1.10). Capsules with no local values create no stub.
